...
#Region " Business Methods "
        ...
        Public Sub AddNewItem(ByVal formField As ClsField, ByVal operation As ClsOperation, ByVal formFieldData As Object)
            'If (formFieldData.IsPrimaryKey OrElse (operation.Description = "Insert" OrElse operation.Description = "Update") AndAlso (formFieldData.OldValue <> formFieldData.NewValue)) AndAlso formField IsNot Nothing Then
            If (formFieldData.IsPrimaryKey OrElse formFieldData.OldValue <> formFieldData.NewValue) AndAlso formField IsNot Nothing Then
                Add(ClsBinnacleFormField.NewChildBinnacleFormField(formField.ID, formFieldData.OldValue, formFieldData.NewValue))
            End If
        End Sub
...
#Region " Child Methods "
        ...
        Friend Shared Function GetChildBinnacleFormFieldList(ByVal list As DAClsprgSubscriberBinnacleFormFields.Struct()) As ClsBinnacleFormFieldList
        ...
        Private Sub New(ByVal list As DAClsprgSubscriberBinnacleFormFields.Struct())
            MarkAsChild()
            Fetch(list)
        End Sub
...
#Region " Common Area "
        ...
        Private Sub Fetch()
            Fetch(DAClsprgSubscriberBinnacleFormFields.Fetch())
        End Sub

        Private Sub Fetch(ByVal idBinnacleForm As SearchCriteria(Of Long), ByVal idField As SearchCriteria(Of Long))
            Fetch(DAClsprgSubscriberBinnacleFormFields.Fetch(New Parameter(Of Long)(idBinnacleForm.Value, idBinnacleForm.Priority), New Parameter(Of Long)(idField.Value, idField.Priority)))
        End Sub

        Private Sub Fetch(ByVal idBinnacleForm As SearchCriteriaList(Of Long), ByVal idField As SearchCriteriaList(Of Long))
            Fetch(DAClsprgSubscriberBinnacleFormFields.Fetch(New ParameterList(Of Long)(idBinnacleForm.Values, idBinnacleForm.Priority), New ParameterList(Of Long)(idField.Values, idField.Priority)))
        End Sub

        Private Sub Fetch(ByVal list As DAClsprgSubscriberBinnacleFormFields.Struct())
