...
#Region " Business Methods "
        ...
        Public Sub AddNewItem(ByVal operation As ClsOperation, ByVal tableFieldData As Object)
            'If tableFieldData.IsPrimaryKey OrElse (operation.Description = "Insert" OrElse operation.Description = "Update") AndAlso (tableFieldData.OldValue <> tableFieldData.Value) Then
...
#Region " Child Methods "
        ...
        Friend Shared Function GetChildBinnacleTableFieldList(ByVal list As DAClsprgAdministrativeBinnacleTableFields.Struct()) As ClsBinnacleTableFieldList
        ...
        Private Sub New(ByVal list As DAClsprgAdministrativeBinnacleTableFields.Struct())
            MarkAsChild()
            Fetch(list)
        End Sub
...
#Region " Common Area "
        ...
        Private Sub Fetch()
            Fetch(DAClsprgAdministrativeBinnacleTableFields.Fetch())
        End Sub

        Private Sub Fetch(ByVal idBinnacleTable As SearchCriteria(Of Long), ByVal fieldName As SearchCriteria(Of String))
            Fetch(DAClsprgAdministrativeBinnacleTableFields.Fetch(New Parameter(Of Long)(idBinnacleTable.Value, idBinnacleTable.Priority), New Parameter(Of String)(fieldName.Value, fieldName.Priority)))
        End Sub

        Private Sub Fetch(ByVal idBinnacleTable As SearchCriteriaList(Of Long), ByVal fieldName As SearchCriteriaList(Of String))
            Fetch(DAClsprgAdministrativeBinnacleTableFields.Fetch(New ParameterList(Of Long)(idBinnacleTable.Values, idBinnacleTable.Priority), New ParameterList(Of String)(fieldName.Values, fieldName.Priority)))
        End Sub

        Private Sub Fetch(ByVal list As DAClsprgAdministrativeBinnacleTableFields.Struct())
            Me.RaiseListChangedEvents = False
            For Each Struct As DAClsprgAdministrativeBinnacleTableFields.Struct In list
