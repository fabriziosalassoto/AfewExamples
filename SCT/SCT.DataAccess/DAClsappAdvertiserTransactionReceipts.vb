...
''' </summary>
''' <remarks></remarks>
<Serializable()> Public Class DAClsappAdvertiserTransactionReceipts
    ...
    Public Shared Function Fetch(ByVal pID As Long) As Struct()
        Fetch = DoFetch(CreateSql("appAdvertiserReceipts.ID = " & pID.ToString))
    End Function
    ...
    Private Shared Function CreateSql(ByVal ParamArray pParametersList() As String) As String
        ...
        Dim sql As String = ""
        sql &= " Select"
        sql &= "  appAdvertiserReceipts.ID,"
        sql &= "  IDAdvertiserProject,"
        sql &= "  IDAdvertiser,"
        sql &= "  " & Transactions.Receipt & " as TransactionType,"
        sql &= "  PaymentDate as TransactionDate,"
        sql &= "  ReceiptNumber as TransactionNumber,"
        sql &= "  TotalPaid as TransactionAmount"
        sql &= " From"
        sql &= "  ("
        sql &= "    appAdvertiserProjects"
        sql &= "   Inner Join"
        sql &= "    appAdvertiserProjectReceipts"
        sql &= "   ON appAdvertiserProjects.ID = IDAdvertiserProject"
        sql &= "  )"
        sql &= " Inner Join"
        sql &= "  appAdvertiserReceipts"
        sql &= " On"
        sql &= "  IDAdvertiserReceipt = appAdvertiserReceipts.ID"
