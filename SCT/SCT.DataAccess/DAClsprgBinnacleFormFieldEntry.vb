...
''' </summary>
''' <remarks></remarks>
<Serializable()> Public Class DAClsprgBinnacleFormFieldEntry
    ...
    ''' <returns>List of DAClsAdminBinnacleFormFields.Struct with the read registries</returns>
    ''' <remarks></remarks>
    Public Shared Function FetchByBinnacleFormEntry(ByVal pLog As Logs, ByVal pIDBinnacleForm As Parameter(Of Long)) As Struct()
        Dim Logs() As Logs = GetLog(pLog)
        FetchByBinnacleFormEntry = DoFetch(Logs, CreateSql(Logs, "IDBinnacleForm = " & pIDBinnacleForm.Value.ToString))
    End Function
