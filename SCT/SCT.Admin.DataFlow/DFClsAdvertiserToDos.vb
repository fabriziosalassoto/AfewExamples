...
Public Class DFClsAdvertiserToDos
...
#Region " Authorization Rules "
    ...
    Public Shared Function CanSelect() As Boolean
        If ClsSessionAdmin.ContainsUserProfileInForm("frmAdvertiserToDos") Then
            Return ClsSessionAdmin.CanSelectInForm("frmAdvertiserToDos")
        Else
    ...
    Public Shared Function CanInsert() As Boolean
        If ClsSessionAdmin.ContainsUserProfileInForm("frmAdvertiserToDos") Then
            Return ClsSessionAdmin.CanInsertInForm("frmAdvertiserToDos")
        Else
    ...
    Public Shared Function CanUpdate() As Boolean
        If ClsSessionAdmin.ContainsUserProfileInForm("frmAdvertiserToDos") Then
            Return ClsSessionAdmin.CanUpdateInForm("frmAdvertiserToDos")
        Else
    ...
    Public Shared Function CanDelete() As Boolean
        If ClsSessionAdmin.ContainsUserProfileInForm("frmAdvertiserToDos") Then
            Return ClsSessionAdmin.CanDeleteInForm("frmAdvertiserToDos")
        Else
    ...
    Public Shared Function CanSelectField(ByVal fieldName As String) As Boolean
        If ClsSessionAdmin.ContainsUserProfileInForm("frmAdvertiserToDos") Then
            Return ClsSessionAdmin.CanSelectFieldInForm("frmAdvertiserToDos", fieldName)
        Else
...
#Region " Properties And Methods "
    ...
    Public Sub ApplyPageAuthorizationRules()
        If (Not ClsSessionAdmin.IsValidForm("frmAdvertiserToDos", "grdAdToDos.ID")) OrElse (Not DFClsAdvertiserToDos.CanSelect()) Then Me.mForm.Response.Redirect("~/Forms/frmMain.aspx")

        Me.mForm.FindControls("grdAdToDos").Columns(6).Visible = DFClsAdvertiserToDo.CanUpdate
        Me.mForm.FindControls("grdAdToDos").Columns(7).Visible = DFClsAdvertiserToDos.CanDelete

        Me.mForm.FindControls("cmdAddNew").FindItem("AddNew").Enabled = DFClsAdvertiserToDos.CanInsert()
    End Sub
    ...
    Public Shared Sub ApplyGridRowAuthorizationRules(ByVal gridView As GridView, ByVal row As System.Web.UI.WebControls.GridViewRow)
        If row.RowType = DataControlRowType.DataRow Then
            Dim canSelectToDo As Boolean = DFClsAdvertiserToDo.CanSelect

            row.Cells(0).Controls(1).Visible = canSelectToDo
            row.Cells(0).Controls(2).Visible = Not canSelectToDo
    ...
    Public Shared Sub HidenMinDate(ByVal gridView As GridView, ByVal row As System.Web.UI.WebControls.GridViewRow)
        If row.RowType = DataControlRowType.DataRow Then
            If row.Cells(4).Text = "01/01/1900" Then
                row.Cells(4).Text = "---"
            End If
        End If
    End Sub
    ...
    Private Function GetDataSources() As DataTable
        ...
        For Each todo As AdToDoData In Me.mDataList
            table.Rows.Add(New Object() {todo.ID, todo.DateEntered, todo.DateCompleted, todo.TaskNotes, todo.Contact.Advertiser.ID, todo.Contact.Advertiser.CompanyName, todo.Contact.ID, todo.Contact.FullName})
        Next
        Return table
    End Function
    ...
    Private Function GetSortProperty() As String
        ...
        Select Case Me.mSortExpression
            Case "DateEntered"
                Return sortProperty & ", DateCompleted " & Me.mSortDirection & ", Advertiser " & Me.mSortDirection & ", Contact " & Me.mSortDirection
            Case "DateCompleted"
                Return sortProperty & ", DateEntered " & Me.mSortDirection & ", Advertiser " & Me.mSortDirection & ", Contact " & Me.mSortDirection
            Case "Advertiser"
                Return sortProperty & ", Contact " & Me.mSortDirection & ", DateEntered " & Me.mSortDirection & ", DateCompleted " & Me.mSortDirection
            Case "Contact"
                Return sortProperty & ", Advertiser " & Me.mSortDirection & ", DateEntered " & Me.mSortDirection & ", DateCompleted " & Me.mSortDirection
            Case Else
                Return sortProperty
        End Select
    End Function
    ...
    Public Sub OnEditing(ByVal adToDoID As String)
        Me.mForm.Session("ID1") = adToDoID
        Me.mForm.Response.Redirect("~/Forms/frmAdvertiserToDo.aspx")
    End Sub

    Public Sub OnDeleting(ByVal adToDoID As String)
        If IsNumeric(adToDoID) AndAlso (Not adToDoID = 0) Then
            Me.mData = New AdToDoData
            Me.mData.ID = adToDoID
            Me.mForm.FindControls("MsgBox").ShowConfirmation("Are you sure you want to delete this ToDo?\n\nNote: there is no undo.", "", True, False)
        End If
    End Sub
...
#Region " Data Methods "
    ...
    Private Function GetDataList(ByVal idAdAccounts() As Long, ByVal idAdContacts() As Long, ByVal fromDate As Date, ByVal toDate As Date) As AdToDoData()
        Try
            Return ClsSessionAdmin.GetAdToDoList(idAdAccounts, idAdContacts, fromDate, toDate)
        Catch SysEx As Exception
            Me.mForm.FindControls("MsgBox").ShowMessage("Error reading: " & SysEx.Message)
