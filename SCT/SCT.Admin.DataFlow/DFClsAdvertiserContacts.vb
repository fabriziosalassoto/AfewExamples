...
Public Class DFClsAdvertiserContacts
...
#Region " Authorization Rules "
    ...
    Public Shared Function CanSelect() As Boolean
        If ClsSessionAdmin.ContainsUserProfileInForm("frmAdvertiserContacts") Then
            Return ClsSessionAdmin.CanSelectInForm("frmAdvertiserContacts")
        Else
    ...
    Public Shared Function CanInsert() As Boolean
        If ClsSessionAdmin.ContainsUserProfileInForm("frmAdvertiserContacts") Then
            Return ClsSessionAdmin.CanInsertInForm("frmAdvertiserContacts")
        Else
    ...
    Public Shared Function CanUpdate() As Boolean
        If ClsSessionAdmin.ContainsUserProfileInForm("frmAdvertiserContacts") Then
            Return ClsSessionAdmin.CanUpdateInForm("frmAdvertiserContacts")
        Else
    ...
    Public Shared Function CanDelete() As Boolean
        If ClsSessionAdmin.ContainsUserProfileInForm("frmAdvertiserContacts") Then
            Return ClsSessionAdmin.CanDeleteInForm("frmAdvertiserContacts")
        Else
    ...
    Public Shared Function CanSelectField(ByVal fieldName As String) As Boolean
        If ClsSessionAdmin.ContainsUserProfileInForm("frmAdvertiserContacts") Then
            Return ClsSessionAdmin.CanSelectFieldInForm("frmAdvertiserContacts", fieldName)
        Else
...
#Region " Properties And Methods "
    ...
    Public Sub New(ByRef form As Object)
        ...
        Me.mDataList = Me.GetDataList(Me.mAdAccountIDs)
        Me.PopulateDataList("FullName", "ASC", 0)
    End Sub

    Public Sub ApplyPageAuthorizationRules()
        If (Not ClsSessionAdmin.IsValidForm("frmAdvertiserContacts", "grdAdContacts.ID")) OrElse (Not DFClsAdvertiserContacts.CanSelect()) Then Me.mForm.Response.Redirect("~/Forms/frmMain.aspx")

        Me.mForm.FindControls("grdAdContacts").Columns(5).Visible = DFClsAdvertiserContact.CanUpdate
        Me.mForm.FindControls("grdAdContacts").Columns(6).Visible = DFClsAdvertiserContacts.CanDelete

        Me.mForm.FindControls("mnuAddNew").FindItem("AddNew").Enabled = DFClsAdvertiserContacts.CanInsert()
    End Sub
    ...
    Public Shared Sub ApplyGridRowAuthorizationRules(ByVal gridView As GridView, ByVal row As System.Web.UI.WebControls.GridViewRow)
        If row.RowType = DataControlRowType.DataRow Then
            ...
            row.Cells(0).Controls(1).Visible = canSelectContact
            row.Cells(0).Controls(2).Visible = Not canSelectContact
    ...
    Private Function GetDataSources() As DataTable
        ...
        For Each contact As AdContactData In Me.mDataList
            table.Rows.Add(New Object() {contact.ID, contact.FullName, contact.Advertiser.ID, contact.Advertiser.CompanyName, contact.PrimaryAddress, "~/Images/Checked_" & contact.MainCompanyAddress.ToString & ".png"})
        Next
        Return table
    End Function
    ...
    Private Function GetSortProperty() As String
        ...
        Select Case Me.mSortExpression
            Case "FullName"
                Return sortProperty & ", Advertiser " & Me.mSortDirection
            Case "Advertiser"
    ...
    Public Sub OnEditing(ByVal adContactID As String)
        Me.mForm.Session("ID1") = adContactID
        Me.mForm.Response.Redirect("~/Forms/frmAdvertiserContact.aspx")
    End Sub

    Public Sub OnDeleting(ByVal adContactID As String)
        If IsNumeric(adContactID) AndAlso (Not adContactID = 0) Then
            Me.mData = New AdContactData
            Me.mData.ID = adContactID
...
#Region " Data Methods "
    ...
    Private Function GetDataList(ByVal idAdAccounts() As Long) As AdContactData()
        Try
            Return ClsSessionAdmin.GetAdContactList(idAdAccounts)
        Catch SysEx As Exception
            Me.mForm.FindControls("MsgBox").ShowMessage("Error reading: " & SysEx.Message)
